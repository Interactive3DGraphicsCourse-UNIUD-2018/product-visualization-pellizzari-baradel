...
function scegliShader(){
	switch(materiale){
		...
		case "oro":
			...
			break;
		case "plastica":
			ShaderLoader("shader/base.vert", ("shader/plastica.frag"),
				function (vertex, fragment) {
					materialeShader = new THREE.ShaderMaterial({
					uniforms: uniformsPlastica,
					vertexShader: vertex,
					fragmentShader: fragment,
					extensions: materialExtensions
					});
...
function interruttoreLuce1(stato){
	if(stato == 'off'){
		lucePuntuale.material.color = new THREE.Color(0.5,0.5,0.5);
		uniformsDinamico.clight.value = new THREE.Vector3(0.0,0.0,0.0);
		uniformsDinamico.clight.needsUpdate = true;
	}else{ // on
		lucePuntuale.material.color = new THREE.Color(1,1,0);
...
function interruttoreLuce2(stato){
	if(stato == 'off'){
		lucePuntuale2.material.color = new THREE.Color(0.5,0.5,0.5);
		uniformsDinamico.clight2.value = new THREE.Vector3(0.0,0.0,0.0);
		uniformsDinamico.clight2.needsUpdate = true;
	}else{ // on
		lucePuntuale2.material.color = new THREE.Color(1,1,0);
